...
const ws_uri = "ws://10.6.1.159:8888/kurento";
...
wss.on('connection', function (ws) {
    ...
    ws.on('message', function (_message) {
        ...
        switch (message.id) {
            case 'client':
                addClient(sessionId, message.sdpOffer, ws, function (error, sdpAnswer) {
                    if (error) {
            ...
            case 'candidate':
                ...
                if (clients[sessionId].webRtcEndpoint) {
                    clients[sessionId].webRtcEndpoint.addIceCandidate(candidate,function(err){
                        console.log("ERRRR",err,candidate);
                    });
                }else {
                    clients[sessionId].candidates.push(candidate);
                }
                break;
...
// Add a webRTC client
function addClient(id, sdp, websocket, callback) {
    ...
    createWebRtcEndPoint(function (error, _webRtcEndpoint) {
        ...
        clients[id].webRtcEndpoint.on("OnIceCandidate", function (candidateEvent) {
            var candidate = kurento.getComplexType("IceCandidate")(candidateEvent.candidate);
        ...
        if(clients[id].candidates.length>0){
            for(var i = 0; i< clients[id].candidates.length ; i++){
                clients[id].webRtcEndpoint.addIceCandidate(clients[id].candidates[i]);
            }
            clients[id].candidates = [];
        }
        createHubPort(function (error, _hubPort) {
            ...
            clients[id].webRtcEndpoint.processOffer(sdp, function (error, sdpAnswer) {
                ...
                clients[id].webRtcEndpoint.gatherCandidates(function (err) {
                    console.log("--------gather error",err);
                });
